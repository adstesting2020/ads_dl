...
public class SlideScreen {
	...
	*/
	...
	public static void slideUp(AppiumDriver driver,int x,int y) {

	int width = driver.manage().window().getSize().width;
	int height = driver.manage().window().getSize().height;	

	new TouchAction(driver).press(PointOption.point(width / 2, x)).waitAction(WaitOptions.waitOptions(duration)).
	moveTo(PointOption.point(width / 2, y)).release().perform();
	}
